...
public class GUIHanoiTowerGame extends SDLMMFrame {
    ...
    public void drawOneHanoi(int[] arr, int x, int y, int widthPerStack,
        ...
        for (int i = 0; i <= topIdx; ++i) {
            ...
            fillRect(x - width / 2, bottom - heightPerStack * (i + 1), width, heightPerStack, 0xff000000 + 0xff123 * arr[i]);
        }
    }
    ...
    private int showSelection(int[] xCenters, int y) {
        ...
        int[] xpt = { xCenters[currentIdx], xCenters[currentIdx] - 10,
                xCenters[currentIdx] + 10 };
        int[] ypt = { y, y + 20, y + 20 };
        fillPolygon(xpt, ypt, 0xff000000);
        switch (pressKey) {
            case 37: // left
    ...
    public void drawHanois(int[] a, int[] b, int[] c) {
        ...
        drawString(String.format("Time Elapsed: %4.1f sec", timeElapsed), 200, 0, 0xff0000ff);
    ...
    public void run() {
        ...
        while (!checkHanoiEnd(c)) {
            drawHanois(a, b, c);
            showMessage("get from which one?");
            if (from == -1) {
                waitSelection = true;
                from = showSelection();
                sleep(1);
