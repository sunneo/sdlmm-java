...
/**
 * 3D N-body simulation using Babylon3D rendering
 * Based on ref-sdlmm nbody.c but with 3D visualization
 */
public class NBody3D extends SDLMMFrame {
    ...
    void init3DRender() {
        device = new Device(SCREENX, SCREENY);
        camera = new Camera();
        camera.Position = new Vector3(0, 0, -cameraDistance);
        camera.Target = new Vector3(0, 0, 0);
        lightPosition = new Vector3(0, 1000, -1000);
        
        // Create sphere meshes for particles
        particleMeshes = new Mesh[SZ];
        for (int i = 0; i < SZ; i++) {
            particleMeshes[i] = Mesh.createSphere(10.0f, 8, 8);
            particleMeshes[i].name = "Particle_" + i;
        }
    }
    ...
    void drawBodys3D(int loop, int totalLoop, double tm, float avgX, float avgY, float avgZ) {
        double rendert1, rendert2;
        String buf;
        rendert1 = getDoubleTime();
        
        // Update camera position (rotate around scene)
        cameraAngle += 0.005f;
        camera.Position = new Vector3(
            (float)(cameraDistance * Math.cos(cameraAngle)),
            avgY,
            (float)(cameraDistance * Math.sin(cameraAngle))
        );
        camera.Target = new Vector3(avgX, avgY, avgZ);
        
        // Update particle positions
        for (int i = 0; i < SZ; i++) {
            if (centralize) {
                particleMeshes[i].Position = new Vector3(
                    SCREENX / 2 * (X_axis[i] / avgX),
                    SCREENY / 2 * (Y_axis[i] / avgY),
                    1000 * (Z_axis[i] / avgZ)
                );
            } else {
                particleMeshes[i].Position = new Vector3(
        ...
        if (showhelp) {
            ...
            buf = String.format("3D Render: %s [3]", use3DRender ? "ON" : "OFF");
    ...
    void drawBodys2D(int loop, int totalLoop, double tm, float avgX, float avgY, float avgZ) {
        int i;
        double rendert1, rendert2;
        String buf;
        rendert1 = getDoubleTime();
        ...
        if (showhelp) {
            ...
            buf = String.format("randomize simulate factor: %s[r]", random_simulatefactor ? "on" : "off");
            drawString(buf, 0, 80, 0xffffffff);
            fillRect(320, 80, (int) (400 * (simulatetime_factor / 2)), 20, 0xfffdfd00);
            drawRect(320, 80, 400, 20, 0xffffffff);
            buf = String.format("centralize: %s[c]", centralize ? "on" : "off");
            drawString(buf, 0, 100, 0xffffffff);
        }
        rendert2 = getDoubleTime();
        flush();
        tm += (rendert2 - rendert1);
    ...
    private void main_run() {
        ...
        for (loop = 0; loop < LOOP; loop++) {
            ...
            if (use3DRender) {
                drawBodys3D(loop, LOOP, fps_time_2 - fps_time_1, avgX, avgY, avgZ);
            } else {
                drawBodys2D(loop, LOOP, fps_time_2 - fps_time_1, avgX, avgY, avgZ);
            }
        }
        tmend = getDoubleTime();
    ...
    public static void main(String[] argv) {
        ...
        NBody3D demo = new NBody3D("NBody3D-Simulation", SCREENX, SCREENY, sz);
